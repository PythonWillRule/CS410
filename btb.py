...
def elapsed(start, end):
    ...
    print("Elapsed time(hh:mm:ss:xx): {:0>2}:{:0>2}:{:05.2f}".format(int(hrs), int(mins), secs))
...
train = pd.read_csv(inputPath+"clicks_train.csv")
...
if evaluating:
    ...
    y = [[_] for _ in y]  # retrieves list of column headers
    p = valid.groupby('display_id').ad_id.apply(list)
    p = [sorted(x, key=get_prob, reverse=True) for x in p]

    print(mapk(y, p, k=12))
else:
    subm = pd.read_csv(inputPath+"sample_submission.csv")

...
print('# File sizes')
inputPath = '/volumes/Macintosh HD/datafiles/'
for f in listdir(inputPath):
    if 'zip' not in f:
        print(f.ljust(30) + str(round(os.path.getsize(inputPath + f) / 1000000, 2)) + 'MB')
print()
# #EndofFile

# ##### Ads #####
df_train = pd.read_csv(inputPath+'clicks_train.csv')
df_test = pd.read_csv(inputPath+'clicks_test.csv')
...
events = pd.read_csv(inputPath + 'events.csv')
...
topics = pd.read_csv(inputPath + 'documents_topics.csv')
...
cat = pd.read_csv(inputPath + 'documents_categories.csv')
...
for i in [1000, 10000, 50000, 100000]:
    print('Number of categories that appear more than {} times: {}'.format(i, (cat_ids > i).sum()) )
